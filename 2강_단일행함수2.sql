...
select * from pd
where regdate >= '2023-03-10' and regdate <= '2023-04-19';

select * from pd
where regdate >= '2023-03-10' and regdate < to_date('2023-04-19') + 1;

--등록한지 몇시간이 지났는지 조회
--pd테이블, regdate이용
select pdname, regdate, (to_date(sysdate)) - (to_date(regdate)) as "경과시간"
from pd;

select sysdate, round(sysdate), trunc(sysdate),
    to_char(sysdate, 'hh24:mi:ss'), to_char(sysdate, 'hh:mi:ss am')
 from dual;
...
--기호를 추가하여 출력하시오.
select name as "이름", hiredate "입사일", to_char((pay*12),'999,999')
    "연봉", to_char((pay*12*1.1), '999,999') "인상 후 연봉"
from professor
where hiredate<'1990-01-01';

--[5] 일반 함수
--nvl(컬럼, 치환할 값) - 해당 컬럼이 null이면 치환할 값으로 바꾸는 함수
select name, bonus, nvl(bonus, 0), hpage, nvl(hpage, '홈페이지 없음')
from professor;
...
--                                 null이면 col3를 출력
select name, bonus, nvl2(bonus, bonus, 0), hpage,
    nvl2(hpage, hpage, '홈페이지 없음'), nvl(bonus, 0)
from professor;

--Professor 테이블에서 101번 학과 교수들의 이름과 급여, bonus, 연봉을 출력하시오. 
--단, 연봉은 (pay*12+bonus)로 계산하고 bonus가 없는 교수는 0으로 계산하시오.
select name, pay, bonus, pay * 12 + bonus, pay * 12 + nvl2(bonus, pay * 12 + bonus, pay*12),
    nvl(pay * 12 + bonus, pay *12), pay * 12 + nvl(bonus, 0)
from professor
where deptno = 101;
...
--nvl()이나 nvl2() 이용
--employees테이블 이용
select first_name || '-' || last_name as "이름", hire_date "입사일", salary "기본급",
    nvl(commission_pct, 0) "수당퍼센트", ((salary + salary) * commission_pct) * 12 "연봉",
   nvl((salary + salary) * commission_pct * 12, salary * 12),
    nvl2(commission_pct,(salary+salary*commission_pct)*12,salary*12) 
from employees;

select first_name || '-' || last_name 이름, hire_date, salary, commission_pct,
...
decode(
    grade, 1, '1학년', 2, '2학년', 3, '3학년', 4, '4학년', '기타'
) as "학년"
from student;
...
select name, deptno, decode(deptno, 101, '컴퓨터공학과', '기타학과') as 학과명
from professor;
...
select name, deptno, decode(deptno, 101, decode(name, '조인형', '석좌교수 후보'))
from professor;	
...
select name, deptno, decode(deptno, 101, decode(name, '조인형', '석좌교수 후보', '후보 아님'), '')
from professor;	
...
select gno, gname, jumin, substr(jumin, 7, 1),
...
-- 055는 경남으로 출력하세요
select name as "이름", tel "전화번호",  substr(tel, 1, instr(tel, ')') -1) "지역번호",
...
select to_char(sysdate), decode(to_char(sysdate, 'd'), '1', '일',
                                                        '2', '월',
                                                        '3', '화',
                                                        '4', '수',
                                                        '5', '목',
...
select name, pay,
    case when pay > 400 then '400 초과'
         when pay between 300 and 400 then '300~400 사이'
         when pay < 300 then '300 미만'
    end "급여범위"
from professor;
...
select profno as "교수번호", name "이름", pay "급여",
    case when pay >= 401 then '1급'
         when pay >= 301 and pay <= 400 then '2급'
         when pay >= 201 and pay <= 300 then '3급'
...
select
    ...
    case 
        when (substr(jumin, 7, 1)) = 1 and (substr(jumin, 7, 1)) = 2 then 1900
        when (substr(jumin, 7, 1)) = 3 and (substr(jumin, 7, 1)) = 4 then 2000
      end
      나이
from gogak; 
...
select 
  ...
  extract(year from sysdate) - (
    ...
    case
      ...
      when substr(jumin, 7, 1) in ('3', '4') then 2000
    end
  ) + 1 "나이"
from gogak;

select gname,jumin, substr(jumin, 1,2), to_char(sysdate, 'yyyy'),
    extract(year from sysdate), substr(jumin,7,1), 
...
select sysdate, to_char(sysdate, 'd'), to_char(last_day(sysdate), 'yyyy mm dd') as "말일",
    to_char(last_day(sysdate), 'day') "말일의 요일",
    decode(to_char(last_day(sysdate), 'd'), '7', to_char(last_day(sysdate), 'yyyy mm dd') -1,
                                            '1', to_char(last_day(sysdate), 'yyyy mm dd') -2,
                                                 to_char(last_day(sysdate), 'yyyy mm dd') 월급날
from dual;
